...
function dayShort(dateText) {
  if (!dateText) return "";
  return new Date(`${dateText}T00:00:00`).toLocaleDateString("en-US", { weekday: "short" });
}

function mdmGroupForClass(className) {
  if (["VI", "VII", "VIII"].includes(className)) return "Junior (VI-VIII)";
  if (["IX", "X"].includes(className)) return "Senior (IX-X)";
  return "Other";
}

function mdmMenuForDay(day) {
  if (day === "Tue" || day === "Sat") return "Soya";
  if (day === "Thu") return "Dal";
  if (day === "Mon" || day === "Wed" || day === "Fri") return "Egg";
  return "Holiday";
}

function calculateMdm(served, dateText, openingRice, receivedRice) {
  const day = dayShort(dateText);
  const menu = mdmMenuForDay(day);
  const servingDay = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].includes(day);
  const riceKg = servingDay ? served * 0.15 : 0;
  const dalKg = menu === "Dal" ? served * 0.03 : 0;
  const soyaKg = menu === "Soya" ? served * 0.025 : 0;
  const eggCount = menu === "Egg" ? served : 0;
  const ladooCount = ["Tue", "Thu", "Sat"].includes(day) ? served : 0;
  const oilKg = servingDay ? served * 0.0075 : 0;
  const fuelAmount = served * 0.7;
  const spiceAmount = servingDay ? served * 0.5 : 0;
  const vegKg = servingDay ? served * 0.1 : 0;
  const totalRice = Number(openingRice || 0) + Number(receivedRice || 0);
  const closingRice = totalRice - riceKg;
  return { day, menu, servingDay, riceKg, dalKg, soyaKg, eggCount, ladooCount, oilKg, fuelAmount, spiceAmount, vegKg, totalRice, closingRice };
}
...
function renderMdm() {
  ...
  const group = $("mdmGroup").value === "Auto" ? mdmGroupForClass(picked.className) : $("mdmGroup").value;
  const calc = calculateMdm(served, $("mdmDate").value, $("mdmOpeningRice").value, $("mdmReceivedRice").value);
  $("mdmEligible").textContent = rows.length;
  $("mdmServed").textContent = served;
  $("mdmAbsent").textContent = rows.length - served;
  $("mdmRiceUsed").textContent = `${numberFmt(calc.riceKg)} kg`;
  $("mdmRiceClosing").textContent = `${numberFmt(calc.closingRice)} kg`;
  $("mdmDay").textContent = calc.day || "-";
  $("mdmMenuAuto").textContent = calc.menu;
  $("mdmRiceTotal").textContent = `${numberFmt(calc.totalRice)} kg`;
  $("mdmCalcRows").innerHTML = [
    ["Group", "Auto from class", group],
    ["Rice", "150 gm/head", `${numberFmt(calc.riceKg)} kg`],
    ["Dal", "30 gm/head on Dal day", `${numberFmt(calc.dalKg)} kg`],
    ["Soya", "25 gm/head on Soya day", `${numberFmt(calc.soyaKg)} kg`],
    ["Egg", "1 egg/head on Egg day", `${numberFmt(calc.eggCount, 0)} pcs`],
    ["Ladoo", "1/head on Tue, Thu, Sat", `${numberFmt(calc.ladooCount, 0)} pcs`],
    ["Oil", "7.5 gm/head", `${numberFmt(calc.oilKg, 3)} kg`],
    ["Fuel", "0.70/head", `Rs ${numberFmt(calc.fuelAmount)}`],
    ["Salt/Spice", "0.50/head", `Rs ${numberFmt(calc.spiceAmount)}`],
    ["Vegetable", "100 gm/head", `${numberFmt(calc.vegKg)} kg`],
    ["Rice Stock", "Opening + Received - Rice Used", `${numberFmt(calc.closingRice)} kg closing`]
  ].map(row => `<tr><td>${escapeHtml(row[0])}</td><td>${escapeHtml(row[1])}</td><td>${escapeHtml(row[2])}</td></tr>`).join("");
  $("mdmStatus").textContent = `${picked.className}-${picked.section}: ${served} meal(s), ${calc.menu}, ${group}.`;
}
...
function renderLogs() {
  ...
  $("mdmLog").innerHTML = (db.mdmEntries || []).length ? db.mdmEntries.map(entry => `<li><strong>${escapeHtml(entry.className)}-${escapeHtml(entry.section)}: ${Number(entry.servedCount || 0)} meals (${escapeHtml(entry.menu || "")})</strong><span>${escapeHtml(entry.date)} ${escapeHtml(entry.day || "")}: Rice ${numberFmt(entry.riceKg)} kg, Dal ${numberFmt(entry.dalKg)} kg, Soya ${numberFmt(entry.soyaKg)} kg, Egg ${numberFmt(entry.eggCount, 0)}, Ladoo ${numberFmt(entry.ladooCount, 0)}, Oil ${numberFmt(entry.oilKg, 3)} kg, Fuel Rs ${numberFmt(entry.fuelAmount)}, Spice Rs ${numberFmt(entry.spiceAmount)}, Veg ${numberFmt(entry.vegKg)} kg, Closing rice ${numberFmt(entry.closingRiceKg)} kg.</span></li>`).join("") : `<li><strong>No MDM entry saved yet</strong><span>Saved entries appear here.</span></li>`;
}
...
async function saveMdm() {
  const picked = currentClass("mdm");
  const calc = calculateMdm(Number($("mdmServed").textContent), $("mdmDate").value, $("mdmOpeningRice").value, $("mdmReceivedRice").value);
  const entry = await api("/api/mdm", {
    method: "POST",
    body: {
      date: $("mdmDate").value,
      ...picked,
      group: $("mdmGroup").value === "Auto" ? mdmGroupForClass(picked.className) : $("mdmGroup").value,
      day: calc.day,
      menu: calc.menu,
      eligibleCount: Number($("mdmEligible").textContent),
      servedCount: Number($("mdmServed").textContent),
      absentCount: Number($("mdmAbsent").textContent),
      openingRiceKg: Number($("mdmOpeningRice").value || 0),
      receivedRiceKg: Number($("mdmReceivedRice").value || 0),
      riceKg: calc.riceKg,
      dalKg: calc.dalKg,
      soyaKg: calc.soyaKg,
      eggCount: calc.eggCount,
      ladooCount: calc.ladooCount,
      oilKg: calc.oilKg,
      fuelAmount: calc.fuelAmount,
      spiceAmount: calc.spiceAmount,
      vegKg: calc.vegKg,
      totalRiceKg: calc.totalRice,
      closingRiceKg: calc.closingRice
    }
  });
...
function bind() {
  ...
  ["mdmClass", "mdmSection", "mdmDate", "mdmGroup", "mdmOpeningRice", "mdmReceivedRice"].forEach(id => $(id).addEventListener("change", renderMdm));
