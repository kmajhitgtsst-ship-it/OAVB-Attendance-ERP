(() => {
  ...
  function downloadBulkTemplate() {
    ...
    Object.entries(bulkTemplateSheets()).forEach(([name, rows]) => {
      const sheet = XLSX.utils.aoa_to_sheet(rows);
      sheet["!cols"] = rows[0].map(header => ({ wch: Math.max(16, Math.min(32, header.length + 4)) }));
  ...
  async function importBulkTemplate(file) {
    ...
    const sheetRows = name => {
      ...
      return rows.length > 1 ? rows.slice(1).map(row => rowObject(rows[0], row)).filter(row => Object.values(row).some(value => text(value))) : [];
    };
    ...
    sheetRows("UC Bulk Entry").forEach(row => {
      ...
      Object.assign(form24, { granteeOrganisation: state.profile.institutionName || "", schemeSector: text(row["Scheme Name and Sector"]) || scheme.name, financialTarget: number(row["Financial Target Fixed"]) || form24.financialTarget || 0, physicalTarget: text(row["Physical Target"]) || form24.physicalTarget || "", presentAchievement: text(row["Physical Achievement"]) || form24.presentAchievement || "", previousUcAmount: text(row["Previous UC Amount"]) || form24.previousUcAmount || "Nil", previousAchievement: text(row["Previous UC Physical Achievement"]) || form24.previousAchievement || "Nil", remarks: text(row.Remarks) || form24.remarks || "" });
      if (text(row["Receipt Financial Year"]) || number(row["Amount Received"])) form24.grantYears.push({ year: text(row["Receipt Financial Year"]), amount: number(row["Amount Received"]) });
      if (text(row["Utilization Financial Year"]) || number(row["Amount Utilized"])) form24.utilizationYears.push({ year: text(row["Utilization Financial Year"]), amount: number(row["Amount Utilized"]) });
      state.form24[key] = form24;
      form24Saved += 1;
    });
    ...
    sheetRows("Form24 Yearwise").forEach(row => {
      ...
      const key = form24Key(scheme.id, financialYear);
      const form24 = state.form24[key] || { financialYear, grantYears: [], utilizationYears: [] };
      Object.assign(form24, { granteeOrganisation: text(row["Grantee Organisation"]) || form24.granteeOrganisation || state.profile.institutionName, schemeSector: text(row["Scheme Name and Sector"]) || form24.schemeSector || scheme.name, financialTarget: number(row["Financial Target Fixed"]) || form24.financialTarget || 0, physicalTarget: text(row["Physical Target Fixed"]) || form24.physicalTarget || "", previousUcAmount: text(row["Previous UC Amount"]) || form24.previousUcAmount || "Nil", previousAchievement: text(row["Previous UC Physical Achievement"]) || form24.previousAchievement || "Nil", presentAchievement: text(row["Present UC Physical Achievement"]) || form24.presentAchievement || "", remarks: text(row.Remarks) || form24.remarks || "" });
      if (text(row["Grant Receipt Financial Year"]) || number(row["Grant Received Amount"])) form24.grantYears.push({ year: text(row["Grant Receipt Financial Year"]), amount: number(row["Grant Received Amount"]) });
      if (text(row["Utilization Financial Year"]) || number(row["Utilized Amount"])) form24.utilizationYears.push({ year: text(row["Utilization Financial Year"]), amount: number(row["Utilized Amount"]) });
      state.form24[key] = form24; form24Saved += 1;
    });
    log("Bulk template imported", `${schemesAdded} schemes, ${receiptsAdded} receipts, ${expensesAdded} expenditures and ${form24Saved} Form-24 rows imported as drafts.`);
    saveState();
    const status = document.getElementById("gm-bulk-template-status");
    if (status) status.textContent = `Imported: ${schemesAdded} schemes, ${receiptsAdded} receipts, ${expensesAdded} expenditures and ${form24Saved} Form-24 rows. Financial entries remain Draft until submitted and approved.`;
    render();
  }
  function bindEvents(root) {
    ...
    document.getElementById("gm-bulk-template-file")?.addEventListener("change", async event => {
      try { await importBulkTemplate(event.target.files?.[0]); }
      catch (error) { alert(error.message || "Template could not be imported."); }
      event.target.value = "";
    });
